...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, inputText: ''}

  getActiveTabApps = () => {
    const {activeTabId} = this.state
    const filteredApps = appsList.filter(app => app.category === activeTabId)
    return filteredApps
  }
  ...
  render() {
    let filteredApps = this.getActiveTabApps()
    const {activeTabId, inputText} = this.state

    if (inputText !== '') {
      const newFilteredApps = filteredApps.filter(each =>
        each.appName.toLowerCase().includes(inputText.toLowerCase()),
      )
        ...
        <ul className="tabs-container">
          {tabsList.map(each => (
            <TabItem
              tabsDetails={each}
              key={each.tabId}
              activeTabIdChanged={this.activeTabIdChanged}
